...
def recode_tu_attr(df, tu_cols, months_value=None):
    """Replace na special value in tu report to nans
    Params
    ------
    df: original input df
    tu_cols: tu attribute col list (with str)
    months_value: list of string feature name for Month Since features

    Return
    ------
    df: data frame after replacing the value
    """
    tu_value = df.loc[:, tu_cols]
    tu_value[tu_value < 0] = np.nan

    if months_value:
        tu_month = df.loc[:, months_value]
        tu_month[tu_month == 999] = np.nan
        tu_value.loc[:, months_value] = tu_month

    df.loc[:, tu_cols] = tu_value

    return df


def recode_oln_attr(df, oln_cols):
    """
    Replace na values from OLN Attributes
    :param df: original input data frame
    :param oln_cols: column list(colname list)
    :return: new data frame after replacing the value
    """
    oln_value = df.loc[:, oln_cols]
    oln_value[oln_value < 0] = np.nan

    df.loc[:, oln_cols] = oln_value
